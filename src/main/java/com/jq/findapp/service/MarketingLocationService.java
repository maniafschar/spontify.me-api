...
public class MarketingLocationService {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if ("updateLocation".equals(poll.type)) {
					final ClientMarketing clientMarketing = this.repository.one(ClientMarketing.class,
							(BigInteger) list.get(i).get("clientMarketing.id"));
					final Client client = this.repository.one(Client.class, clientMarketing.getClientId());
					final String html = this.createHtmlTemplate(client);
					params.setSearch(
					...
					final Result locations = this.repository.list(params);
					...
					for (int i2 = 0; i2 < locations.size(); i2++) {
						...
						final String subject = this.text.getText(contact,
						...
						if (!sent) {
							...
							final String body = this.text.getText(contact,
									TextId.valueOf("marketing_" + poll.locationPrefix + "Text"))
									+ this.text.getText(contact,
											TextId.valueOf("marketing_" + poll.locationPrefix + "Postfix"));
							this.notificationService.sendEmail(client, null, location.getEmail(),
	...
	public CronResult cronUnfinished() {
		return this.sendEmails("contactMarketing.finished=false and contactMarketing.createdAt>cast('"
	...
	private CronResult sendEmails(final String search, final String subject, final String postfixText,
		...
		params.setSearch(search + " and (contactMarketing.status is null or contactMarketing.status not like '%"
				+ postfixText + "%')");
		try {
			final Result contactMarketings = this.repository.list(params);
			...
			for (int i = 0; i < contactMarketings.size(); i++) {
				...
				if (answer.has("locationId") && clientMarketing.getEndDate().getTime() > end) {
					...
					if (decide.apply(location, poll, answer)) {
						...
						final String subject2 = this.text.getText(contact,
								TextId.valueOf("marketing_" + poll.locationPrefix + "SubjectPrefix")) + subject;
						final ContactMarketing contactMarketing = this.repository.one(ContactMarketing.class,
						...
						if (!htmls.containsKey(client.getId()))
							htmls.put(client.getId(), this.createHtmlTemplate(client));
	...
	String createHtmlTemplate(final Client client) {
		String html;
		try (final InputStream inHtml = this.getClass().getResourceAsStream("/template/email.html")) {
	...
	public String pollFinished(final ContactMarketing contactMarketing) {
		...
		if (answers.has("locationId") &&
			...
			if ("updateLocation".equals(poll.type))
				return this.update(contactMarketing, clientMarketing, poll, answers, location);
			return this.createEvents(contactMarketing, clientMarketing, poll, answers, location);
		}
		return null;
	}
	...
	private String createEvents(final ContactMarketing contactMarketing, final ClientMarketing clientMarketing,
		...
		if (answers.get("q0").has("a")) {
			...
			if (location.getContactId() == null || location.getContactId().intValue() < 10) {
				try {
					location.setContactId(this.createUser(clientMarketing.getClientId(), location));
					this.repository.save(location);
					...
					this.notificationService.createTicket(TicketType.ERROR, "MarketingLocation",
	...
	private String update(final ContactMarketing contactMarketing, final ClientMarketing clientMarketing,
		...
		if ((location.getUpdatedAt() == null || location.getUpdatedAt().before(clientMarketing.getStartDate()))
			...
			this.notificationService.sendEmail(client, null,
					location.getEmail(),
					"Deine Location " + location.getName(), email,
					this.createHtmlTemplate(this.repository.one(Client.class, clientMarketing.getClientId()))
	...
	private BigInteger createUser(final BigInteger clientId, final Location location) throws Exception {
		...
		return this.authenticationService.register(registration).getId();
	}
}
