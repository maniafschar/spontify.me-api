	...
	class Synchonize {
		...
		private void predictionAddStatistics(final BigInteger clientId, final PollMatchDay poll) throws Exception {
			...
			for (int i = 0; i < matches.size(); i++) {
				...
				if (i < 8)
					poll.matches.add(s + "|"
							+ formatDate((Long) matches.get(i).get("timestamp"), null));
			}
