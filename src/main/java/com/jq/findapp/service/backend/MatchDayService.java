...
public class MatchDayService {
	...
	public String retrieveMatchDays(final int pastMatches, final int futureMatches, final List<Integer> teamIds) {
			...
			if (matchDays != null) {
				for (int i = 0; i < matchDays.size(); i++) {
					...
					matches.put(timestamp + "." + teamId, "<span><header>" + leagueName + " · " + venue + " · " + city + " · " + formatDate(timestamp, null) + "</header>"
							+ "<home>" + homeName + "</home><goals>" + homeGoals + "</goals><goals class=\"sep\">" + awayGoals + "</goals><away>" + awayName + "</away></span>");
				}
			}
		}
		...
		s.insert(0, "<style>header{font-size:0.7em;}span{padding-bottom:1em;display:block;}home,away{width:40%;display:inline-block;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;}home{text-align:right;}away{text-align:left;}goals{width:10%;display:inline-block;text-align:center;overflow:hidden;}goals.sep::before{content:':';}</style><matchDays style=\"text-align:center;\">");
		s.append("</matchDays>");
		return s.toString();
	}
