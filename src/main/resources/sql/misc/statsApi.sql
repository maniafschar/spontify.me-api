SELECT
	...
	count(*)*1.0/(select count(*) from Log l where l.clientId={CLIENTID} and l.uri like '/%' and l.uri not like '/support/%' and l.uri not like '/ws/%' and cast(TO_DAYS(createdAt) as integer)>cast(TO_DAYS(current_timestamp) as integer)-90) as percentage
FROM
	Log log
WHERE
